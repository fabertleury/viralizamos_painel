...
async function testConnections() {
  ...
  try {
    ...
    const ordersResult = await ordersPool.query('SELECT COUNT(*) FROM "users"');
    ...
    const userResult = await ordersPool.query('SELECT id, email FROM "users" LIMIT 1');
    if (userResult.rows.length > 0) {
      ...
      if (pagamentosUserResult.rows.length > 0) {
        ...
        const transactionsQuery = `
          ...
          FROM "transactions" t
          JOIN "payment_request" pr ON t.payment_request_id = pr.id
          WHERE pr.user_id = $1
        `;
